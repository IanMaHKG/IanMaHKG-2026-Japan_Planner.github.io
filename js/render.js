 ...
 * by reading from data modules and building semantic HTML.
 */
...
function renderCarRental() {
  const data = (typeof window !== 'undefined' && window.SITE_DATA) ? window.SITE_DATA.carRental : null;
  if (!data) return;
...
function renderCarReturn() {
  const data = (typeof window !== 'undefined' && window.SITE_DATA) ? window.SITE_DATA.carReturn : null;
  if (!data) return;
    ...
    gridEl.innerHTML = data.options.map(opt => {
      ...
      return `
        <div class="cr-card verdict-${opt.verdict}" id="${opt.id}">
          ...
          ${tipsHtml ? `
          <div class="cr-tips-toggle" onclick="toggleCarTips(this)" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();toggleCarTips(this);}" role="button" tabindex="0" aria-expanded="false">
...
function renderOverview() {
  const data = (typeof window !== 'undefined' && window.SITE_DATA) ? window.SITE_DATA.overview : null;
  if (!data) return;
...
function renderTips() {
  const tips = (typeof window !== 'undefined' && window.SITE_DATA) ? window.SITE_DATA.tips : null;
  const tipsEl = document.getElementById('tips-grid');
  if (!tipsEl || !tips) return;
...
function renderPacking() {
  const packing = (typeof window !== 'undefined' && window.SITE_DATA) ? window.SITE_DATA.packing : null;
  const packEl = document.getElementById('packing-grid');
  if (!packEl || !packing) return;

  packEl.innerHTML = packing.map(pack => {
...
function renderBudget() {
  const data = (typeof window !== 'undefined' && window.SITE_DATA) ? window.SITE_DATA.budget : null;
  const tbodyEl = document.getElementById('budget-tbody');
  if (!tbodyEl || !data) return;
...
function renderHotels() {
  const data = (typeof window !== 'undefined' && window.SITE_DATA) ? window.SITE_DATA.hotels : null;
  if (!data) return;
...
function renderItinerary() {
  ...
  const itin = (typeof window !== 'undefined' && window.ITINERARY_DATA) ? window.ITINERARY_DATA : null;
  if (!container || !itin) return;

  const html = itin.map(day => {
    ...
    return `
      ...
      <div class="day-card" data-region="${day.region}" id="${day.id}">
        <div class="day-header" onclick="toggleDay(this)" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();toggleDay(this);}" role="button" tabindex="0" aria-expanded="false">
          <div class="day-badge">
...
function toggleDay(header) {
  ...
  document.querySelectorAll('.day-card.open').forEach(c => {
    if (c !== card) {
      ...
      const hdr = c.querySelector('.day-header');
...
if (typeof window !== 'undefined') {
  ...
  window.renderCarReturn    = renderCarReturn;
  window.renderCarRental    = renderCarRental;
  window.renderTips         = renderTips;
  window.renderPacking      = renderPacking;
